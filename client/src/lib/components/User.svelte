...
{:else if formView === "register"}
    ...
    onsubmit={fetchUser}>
        <input type="text" placeholder="Username" name="username" bind:value={username} />
        <button type="submit" disabled={isSubmit === true || username === ""}>Register</button>
    </form>
{/if}